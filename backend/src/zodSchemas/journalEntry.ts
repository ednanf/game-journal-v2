...
const journalEntryBaseSchema = z.object({
    ...
    status: z.enum(['started', 'completed', 'revisited', 'paused', 'dropped']),
...
const createJournalEntryBodySchema = journalEntryBaseSchema
    .refine(
        (data) => data.status !== 'completed' || data.rating !== undefined,
        {
            path: ['rating'],
    ...
    .refine(
        ...
        {
            path: ['rating'],
            message: 'Rating is only allowed for completed entries.',
        },
    )
    .strict();

const patchJournalEntryBodySchema = journalEntryBaseSchema.partial().strict();
